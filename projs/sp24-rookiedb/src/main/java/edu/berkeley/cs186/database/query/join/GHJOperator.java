...
public class GHJOperator extends JoinOperator {
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        // SHJ: relies on R to fit into memory (R <= B-2 pages)
        // GHJ: hash R and S into B-1 buffers such that partitions are <= B-2 pages
        // large
        // The number of records to make SHJ broken but not GHJ
        // = number of partitions * number of pages/ partition * number of records /
        // page + 1
        // = (B - 1) * (B - 2) * 8 + 1
        // = 5 * 4 * 8 + 1 = 161
        for (int i = 0; i < 161; i++) {
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        // GHJ: break when could not recursively reduce the partition size <= B-2 pages
        // It happens when all records are identical in the partition.
        // The number of records to make GHJ broken
        // = number of partitions * number of pages/ partition * number of records /
        // page + 1
        // = 1 * (B - 2) * 8 + 1 = 1 * 4 * 8 + 1 = 33 records
        for (int i = 0; i < 33; i++) {
